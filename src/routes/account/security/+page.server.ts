...
export const load: PageServerLoad = async ({
	locals: { supabase, providers, aal },
	setHeaders
}) => {
	setHeaders({ 'cache-control': 'private, no-store' });
	const [factors, sessions] = await Promise.all([
		supabase.auth.mfa.listFactors(),
		providers.database.rpc('get_account_sessions')
	]);
	return {
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		if (intent === 'revokeSession') {
			...
			const { data, error } = await locals.providers.database.rpc('revoke_account_session', {
		...
		const { error: accessError } = await locals.providers.database.rpc('get_account_sessions');
		if (accessError)
			return fail(403, {
