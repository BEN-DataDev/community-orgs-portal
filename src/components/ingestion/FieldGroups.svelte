	...
	{#if group.length}
		<details class="disclosure">
			<summary class="cursor-pointer">
				...
				{#if selectable}<span class="ml-2 text-sm"
						>{group.filter((f) => selected.includes(JSON.stringify(f))).length} selected</span
					>{/if}
				<span class="block text-sm font-normal">
					...
					{group.filter((f) =>
						['missing', 'unmapped', 'suppressed', 'unchanged'].includes(f.status)
					).length} excluded
				</span>
			</summary>
			<fieldset class="fieldset disclosure-content space-y-3" aria-label={`${section} fields`}>
				...
				<div class="table-wrap">
					<table class="table text-sm">
						<thead
							><tr><th>Field</th><th>Current</th><th>Proposed</th><th>Assessment</th></tr></thead
						>
						<tbody>
