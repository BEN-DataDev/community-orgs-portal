...
| U04 — Medium           | P32 adds scoped edit actions and controls for existing aliases, document links, locations and relationships, including end dates. The hosted signed-in regression passed location add/edit/remove and alias/relationship add/edit. Scoped alias and relationship removal were added locally afterward and await hosted verification. Documents accept URLs, not local file uploads. | **Deployed with local follow-up awaiting verification.** Removal actions recheck editor access and scope by record plus organisation. Treat local document upload as a separate storage/access/retention feature; the implemented workflow manages links only. Relationship partners remain names, so linked identity is still a separate model decision. |
